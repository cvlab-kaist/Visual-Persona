...
def add_text_to_image(
    ...
    for i, line in enumerate(lines):
        ...
        if add_below:
            draw.text(
                (text_x, h + y_offset + offset * i), line, font=font, fill=text_color
            )
        else:
            draw.text(
                (text_x, 0 + y_offset + offset * i), line, font=font, fill=text_color
            )
    return np.array(img)


def create_table_plot(
    titles: List[str], images: List[Image.Image], captions: List[str]
) -> Image.Image:
    title_max_lines = np.max(
        [len(textwrap.wrap(text, width=LINE_WIDTH)) for text in titles]
    )
    caption_max_lines = np.max(
        [len(textwrap.wrap(text, width=LINE_WIDTH)) for text in captions]
    )
    out_images = []
    for i in range(len(images)):
        im = np.array(images[i])
        im = add_text_to_image(
            im, titles[i], add_below=False, min_lines=title_max_lines
        )
        im = add_text_to_image(
            im, captions[i], add_below=True, min_lines=caption_max_lines
        )
