...
REFERNCE_FACIAL_POINTS_RELATIVE = (
    np.array(
        [
...
def verify_load(missing_keys, unexpected_keys):
    if unexpected_keys:
        RuntimeError(
            f"Found unexpected keys in state dict while loading the encoder:\n{unexpected_keys}"
        )

    filtered_missing = [key for key in missing_keys if not "extract_kv1" in key]
    if filtered_missing:
        RuntimeError(
            f"Missing keys in state dict while loading the encoder:\n{filtered_missing}"
        )
...
def extract_faces_and_landmarks(
    ...
    refernce_points=REFERNCE_FACIAL_POINTS_RELATIVE,
):
    """
        ...
        if landmarks is None:
            ...
            affine_transformations.append(
                get_similarity_transform_for_cv2(
                    landmarks[0].astype(np.float32),
                    refernce_points.astype(np.float32) * output_size,
                )
            )
    affine_transformations = torch.from_numpy(
        np.stack(affine_transformations).astype(np.float32)
    ...
    return (
        warp_affine(
            fp_images, affine_transformations, dsize=(output_size, output_size)
