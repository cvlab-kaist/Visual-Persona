...
class Config:
    ...
    inputs_path: Path = Path("data_inference/evaluation/images")
    full_mask_path: Path = Path("data_inference/evaluation/full_body_masks")
    part_mask_path: Path = Path("data_inference/evaluation/body_part_masks")
    out_dir: Path = Path("./results/controlnet")

    control_paths = Path("data_inference/control_images")
    ...
    # Prompt settings
    with open("data_inference/control_images/control_prompts.txt", "r") as f:
...
Images per prompt: {cfg.num_images_per_prompt}"""
    ...
    for control_path in tqdm(control_paths, desc=" images", position=0):
        ...
        control_imgs.append(openpose_control_image)
    ...
    for path in tqdm(paths, desc=" Actors", position=0):
        ...
        for ctrl_num, openpose_image in enumerate(control_imgs):
            openpose_image.save(image_out_dir / f"control_{ctrl_num}.png")
