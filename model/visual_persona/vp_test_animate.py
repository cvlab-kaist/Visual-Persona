...
class Visual_Persona_Animate:
    ...
    def load_ip_adapter(self):
        ...
        if "unet" in state_dict.keys():
            for key, value in state_dict["unet"].items():
                setattr(self.pipe.unet, key, value)
        
        self.image_proj_model.load_state_dict(state_dict["image_proj"])
        ip_layers = torch.nn.ModuleList([
            value for key, value in self.pipe.unet.attn_processors.items() 
