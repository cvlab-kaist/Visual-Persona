    ...
    def __call__(
        ...
        **kwargs,
        ...
        if encoder_hidden_states is None:
            ...
            # ip_hidden_states = torch.Size([2, 1024, 2048])
            if attn.norm_cross:
                encoder_hidden_states = attn.norm_encoder_hidden_states(
                    encoder_hidden_states
                )
        ...
        key = attn.to_k(
            encoder_hidden_states
        )  # 1, 77, 2048 -> 1 77 640 (channel * head_dim) # torch.Size([2, 77, 640])
        value = attn.to_v(encoder_hidden_states)

        inner_dim = key.shape[-1]  # 640
        head_dim = inner_dim // attn.heads  # 64 (640 / 10)
        ...
        hidden_states = F.scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        )  # 1, 4096, 640 # torch.Size([2, 10, 4096, 64])

        hidden_states = hidden_states.transpose(1, 2).reshape(
            batch_size, -1, attn.heads * head_dim
        )  # torch.Size([2, 4096, 640])
        hidden_states = hidden_states.to(query.dtype)

        # for ip-adapter
        ip_key = self.to_k_ip(
            ip_hidden_states
        )  # 1, 48, 640  # torch.Size([2, 1024, 640])
        ip_value = self.to_v_ip(ip_hidden_states)  # torch.Size([2, 1024, 640])

        ip_key = ip_key.view(batch_size, -1, attn.heads, head_dim).transpose(
            1, 2
        )  # 1, 10, 48, 64 # torch.Size([2, 10, 1024, 64])
        ip_value = ip_value.view(batch_size, -1, attn.heads, head_dim).transpose(
            1, 2
        )  # torch.Size([2, 10, 1024, 64])
