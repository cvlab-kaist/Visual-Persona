...
class TransformerDecoder(nn.Module):
    ...
    def _block_fn(self, inner_dim, cond_dim, mod_dim):
        ...
        if self.block_type == "basic":
        ...
        elif self.block_type == "cond_self":
            assert (
                cond_dim is not None
            ), f"Condition dimension must be specified for ConditionBlock"
            assert (
            ...
            from .block import ConditionBlock_self

            # logger.debug(f"Using ConditionBlock")
            return partial(ConditionBlock_self, inner_dim=inner_dim, cond_dim=cond_dim)
        elif self.block_type == "cond_mlp":
            assert (
                cond_dim is not None
            ), f"Condition dimension must be specified for ConditionBlock"
            assert (
            ...
            from .block import ConditionBlock_mlp

            # logger.debug(f"Using ConditionBlock")
            return partial(ConditionBlock_mlp, inner_dim=inner_dim, cond_dim=cond_dim)
    ...
    def forward(
        self, x: torch.Tensor, cond: torch.Tensor = None, mod: torch.Tensor = None
    ):
        # x: [N, L, D]
